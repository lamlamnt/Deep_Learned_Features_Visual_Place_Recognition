...
#"2014-12-16-09-14-09" - added in later
runs_time = ["2015-02-20-16-34-06","2014-12-10-18-10-50","2014-12-17-18-18-43","2015-05-22-11-14-30",
...
#Copies theses frames into the new folder
    for run in runs_time:
        ...
        file_path = os.path.join("/Volumes/scratchdata/lamlam/processed_data",next((key for key, value in runs.items() if value == run), None)
 + "_" + run,file_name)
        chosen_frames = []
        with open(file_path,"r") as f:
            for line in f:
        ...
        existing_folder_path = os.path.join("/Volumes/scratchdata/lamlam/processed_data",next((key for key, value in runs.items() if value == run), None)
 + "_" + run)
